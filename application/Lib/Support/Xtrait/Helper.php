...
use Lib\Dao\System\OptionDao;
use Lib\Dao\System\OptionGroupDao;
use Trensy\Shortcut;
use Trensy\Context;
...
trait Helper
{
    ...
    public static function getDbOption($gk, $vk)
    {
        $options = self::getDbOptions($gk);
        return isset($options[$vk])?$options[$vk]:null;
    }

    public static function getDbOptions($gk)
    {
        $key = __CLASS__ . ":" . __METHOD__ . "(" . $gk . ")";

        return self::fetchCached($key, $gk, function ($gk) {
            $groupDao = new  OptionGroupDao();
            $id = $groupDao->getField("id", ['group_key' => $gk]);
            if (!$id) return false;
            $optionDao = new OptionDao();
            return $optionDao->getField("option_value", ['group_id' => $id], true, "option_key");
        });
    }

    public static function clearDbOptions($gk)
    {
        $key = "Helper:getDbOptions(" . $gk . ")";
        $redis = self::cache();
        return $redis->del($key);
    }

    public static function getOlineIp()
    {
...
//        debug($host);
        $key = "lua_cname_key".$host;
        $customeDomain = self::redis()->get($key);
        if($customeDomain){
            $host = $customeDomain;
        }
        return $host;
    ...
    /**
     *
     * $this->fetchCached("id:{$id}", $id, function ($id) use ($that) {
    ...
    /**
     *
     * $this->fetchFileCached("id:{$id}", $id, function ($id) use ($that) {
    $sql = "SELECT * FROM {$that->getTable()} WHERE id = ? LIMIT 1";
    return $that->getConnection()->fetchAssoc($sql, array($id)) ?: null;
    })
     *
